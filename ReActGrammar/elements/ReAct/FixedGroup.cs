...
{
    class FixedGroup : AGene
    {
        public FixedGroup(Configuration config,object value) 
            : base (config, GeneType.ActionPattern, ReturnType.Bool,value)

        {
            childenReturnType = ReturnType.Bool;
        }
        public FixedGroup(IChromosome chrom, Configuration config, object value)
            : this(config, value)
        {
            SetChromosome(chrom);
        }
        ...
        {
            // if I want to shift ReAct closer to BT it is possible to treat APs as sequence nodes allowing other types of genes as well
            for (int i = 0; i < children.Length; i++)
                if (children[i] is ReActAction)
        ...
        {
            return new FixedGroup(gpConfig,value);
        }
